...
;(load "C:\\Users\\Davide\\Desktop\\LISP\\sudoku.lsp")
...
; applica quindi la procedura SetRowBoolCorr ad ongi riga.
...
;toglie il primo numero da ogni riga.
...
;	val indica il passo attuale (o 0 o 1).
;ritorna:
...
;--------------------------------------------------------------------
;ContaIndice trova l'indice in cui inserire il numero nella lista di 9 posizioni.
;Parametri:
;	la lista di booleani
...
defun ContaIndice(val lbool)
	...
	if (not (= 0 val))
		(
		if (not (car lbool))
			(+ 1 (ContaIndice val (cdr lbool)))
			(+ 1 (ContaIndice (- val 1) (cdr lbool)))
		)
...
;es:
;(ContaIndice 4 (list T T NIL NIL T T))
					...
					progn
						...
						if (not (RisolviSudoku mOrig 
							...
							progn
								...
								;se è falso allora cambia posizione. quella attuale non è più valida.
								;richiama la funzione con semplicemente la matrice dei booleani con la posizione attuale a NIL.
								(RisolviSudoku mOrig m (MatrixChangeRow mBool (InserisciNum 0 (TrovaRiga mBool row) (TrovaRiga mBool row) NIL) row) num row)
							)
						)
					)
				)
